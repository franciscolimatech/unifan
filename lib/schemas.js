 ...
 *   tipo              String    ("post")
 *   postId            ObjectId  (ref: posts._id, indexado)
 *   remetenteId       ObjectId  (ref: users._id, indexado)
 *   remetenteNome     String
 *   destinatarioId    ObjectId  (ref: users._id, indexado)
 *   destinatarioEmail String    (normalizado para auditoria interna)
...
async function criarIndices() {
  ...
  // shared_items
  await db.collection('shared_items').createIndex(
    { destinatarioId: 1, criadoEm: -1 },
    { name: 'idx_shared_items_destinatario_data' }
  );
  await db.collection('shared_items').createIndex(
    { remetenteId: 1, criadoEm: -1 },
    { name: 'idx_shared_items_remetente_data' }
  );
  await db.collection('shared_items').createIndex(
    { postId: 1, criadoEm: -1 },
    { name: 'idx_shared_items_post_data' }
  );
